...
xInstallTarget pd lbi cf fn = do
  ...
  _ <- flip mapM extended $ \exe -> do
    ...
    let lbi' = patchLibexecdir lbi
        ...
        progprefix = substPathTemplate (packageId pd) lbi "ghc-mod-5.8.0.0" (progPrefix lbi)
        progsuffix = substPathTemplate (packageId pd) lbi "ghc-mod-5.8.0.0" (progSuffix lbi)
